...
for (x in 1:length(Select_pathways)) {
  ...
  pathway_genes <- GFAP_enriched_pathways$protein_labels[
    grep(Select_pathways[x], GFAP_enriched_pathways$term.description, ignore.case=TRUE)
  ]
...
heatmap_matrix <- matrix(NA, nrow = ncol(all_pathway_genes), ncol = length(unique(GFAP_res_expanded$Genes)), 
                         dimnames = list(colnames(all_pathway_genes), unique(GFAP_res_expanded$Genes)))
...
for (pathway_name in pathway_names) {
  ...
  for (gene in pathway_genes) {
    if (gene %in% GFAP_res_expanded$Genes) {
      logFC_values <- GFAP_res_expanded$logFC[GFAP_res_expanded$Genes == gene]
...
filename1 <- paste0(pathResults3, "Heatmap_all_shared_GFAP_ALDH1L1_pathways_selectdatabase2.pdf")
