...
pathResults2 <- ("C:\\Users\\teres\\OneDrive - Nexus365\\MSc Oxford\\Hilary Project\\Proteomics\\Astrocytes\\results\\vsnNorm\\outlierRM_GFAP\\Figures\\")
...
GFAP_res$category <- with(GFAP_res, ifelse(adj.P.Val < 0.05 & logFC > 1, "Upregulated",
                                  ifelse(adj.P.Val < 0.05 & logFC < -1, "Downregulated",
                                         NA))) # Set non-significant to NA

# Filter out NA categories (non-significant genes)
GFAP_res_filtered <- GFAP_res[!is.na(GFAP_res$category), ]

# Count the number of genes in each category
category_counts <- table(GFAP_res_filtered$category)
...
filename1 <- paste0(pathResults2, "No_GFAP_DEPs.png")
...
filename2 <- paste0(pathResults2, "No_GFAP_DEPs.pdf")
